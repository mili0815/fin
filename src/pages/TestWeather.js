...
function getRegionRegId(cityName, stateName) {
  if (cityName.startsWith("동탄")) return cityToRegId["화성시"];
  if (cityName in cityToRegId) return cityToRegId[cityName];
  if (stateName === "경기도") return cityToRegId["수원시"];
  return cityToRegId["서울특별시"];
}

function getDisplayCityName(cityName) {
  if (cityName.startsWith("동탄")) return "동탄(화성시)";
  if (cityToRegId[cityName]) return cityName;
  return "서울특별시";
}
export default function WeatherPage() {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(async ({ coords }) => {
      ...
      setCity(getDisplayCityName(cityName));
      const regId = getRegionRegId(cityName, stateName);
      try {
        // 단기예보 호출
        ...
        // currentTemp / currentHumid fallback
        if (currentTemp == null) {
        ...
        console.log(midItem);
        // 4) 중기예보 요약: getMidLandFcst, 예: 오후 요약 wf4Pm~wf7Pm
    ...
    <Container>
      <Content>
        ...
        {weather.current != null ? (
          <>
            <TempInfo>
              <strong>현재 온도: {weather.current}°C</strong>
              <div>체감 온도: {weather.feelsLike}°C</div>
              <div>
                최고: {weather.tmx}°C / 최저: {weather.tmn}°C
              </div>
              ...
              <Row>
                {weather.hourly.map((h, i) => (
                  <ForecastBox key={i}>
                    ...
                    <div style={{ fontSize: "14px" }}>{h.temp}°C</div>
                  </ForecastBox>
                ))}
              </Row>
            </Card>
